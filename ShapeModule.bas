...
'What it does is scan the Image passed to it and then remove all lines that
...
'Scan a line of transparent pixels
                While X <= BM.bmWidth And GetPixel(hDC, X, Y) = cTransparent
                    X = X + 1
                Wend
'Create a new Region that corresponds to the row of Transparent pixels and then
